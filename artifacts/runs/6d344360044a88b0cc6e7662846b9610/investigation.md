...
| Token lifecycle | (external — Railway dashboard) | Partial | The 10-occurrence recurrence pattern (9 unique issues, #762 fired twice) shows that prior rotations accepted Railway's default finite TTL instead of selecting **No expiration**. The repo cannot fix this; only a human at https://railway.com/account/tokens can. |
