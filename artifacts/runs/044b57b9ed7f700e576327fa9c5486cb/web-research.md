...
Issue #781 is the **14th `archon:in-progress` recurrence** in this repo of "Prod deploy failed on main / RAILWAY_TOKEN is invalid or expired: Not Authorized". The broader set of related issues observed over the recurrence window includes #725, #727, #728, #733, #737, #739, #742, #745, #747, #748, #751, #752, #755, #762, #769, #771, #773, #774, #777, #779 — not all of these gained the `archon:in-progress` label or were investigated by this workflow. The canonical 14-recurrence lineage that *was* investigated is in `investigation.md` § "Git History". Web research suggests two distinct root-cause hypotheses worth investigating: (a) Railway's dashboard-created account tokens may have an undocumented or short default TTL, and (b) the validation query (`{me{id}}`) and `Authorization: Bearer` header pin the workflow to an *account-scoped* personal access token specifically — workspace tokens (the type Railway recommends "Best For Team CI/CD") will be rejected by that validation step even though they could perform the actual deploy mutation. The recurring rotation pattern strongly indicates the operator is creating a token type whose lifecycle is not under our control rather than a token mis-scoped against our workflow.
