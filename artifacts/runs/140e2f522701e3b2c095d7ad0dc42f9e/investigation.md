# Investigation: Prod deploy failed on main (26th RAILWAY_TOKEN expiration)

**Issue**: #816 (https://github.com/alexsiri7/reli/issues/816)
**Type**: BUG
**Investigated**: 2026-05-01T00:45:00Z
...
| Confidence | HIGH | Run log explicitly emits `RAILWAY_TOKEN is invalid or expired: Not Authorized` (timestamp `2026-04-30T23:04:46.5253658Z`), the validate step at `.github/workflows/staging-pipeline.yml:32-58` is designed to surface exactly this case, and the same failure has now recurred 26 times. Authoritative chain: `git log --grep "RAILWAY_TOKEN expiration" --oneline` on `main` (most recent: #815 = 25th; #813 = 24th; #812 = 23rd; #809 = 22nd; #806 = 21st). |

---

## Problem Statement

The `Deploy to staging` job in `.github/workflows/staging-pipeline.yml` fails at the `Validate Railway secrets` step because the `RAILWAY_TOKEN` GitHub Actions secret has expired again. Railway's GraphQL API rejects the token with `Not Authorized`, the deploy step is skipped, and downstream `Staging E2E smoke tests` and `Deploy to production` jobs are skipped as well — which is what `pipeline-health-cron.sh` reports as "Prod deploy failed on main". **Agents cannot fix this** — rotation requires a human with railway.com dashboard access.
...
This is the **26th recurrence** of the same `RAILWAY_TOKEN` expiration. The validate step is working as designed; it is correctly surfacing an expired credential. The structural driver of the recurrence cycle (per prior investigations and Railway's published guidance at `docs.railway.com/integrations/api`) is most likely the use of an **account-tier** token (user-bound, easily revoked) and/or a default short-lived TTL on each rotation. Railway officially recommends **workspace tokens** with no expiration for "Team CI/CD" — they are a drop-in replacement (same `Authorization: Bearer` header, same `{me{id}}` validation works unchanged).

Notably, the failing run was triggered on SHA `add83e55f871e009b802c1b776619ff7f1204055`, which is the merge commit of PR #813 — the investigation for the **24th** recurrence. The 25th investigation (#814/#815, merged as `d511b80`) has since landed on `main`, and that merge will fail the staging pipeline in exactly the same way until the token is rotated.

### Evidence Chain

WHY: The "Staging → Production Pipeline" run #25193675022 ends `failure`, and downstream jobs (`Staging E2E smoke tests`, `Deploy to production`) were `skipped`.
↓ BECAUSE: The `Deploy to staging` job's `Validate Railway secrets` step exited 1.
  Evidence: `gh run view 25193675022` — `jobs[Deploy to staging].steps[Validate Railway secrets].conclusion = failure`; `Deploy staging image to Railway`, `Wait for staging health`, `Staging E2E smoke tests`, and `Deploy to production` all `skipped`.

↓ BECAUSE: A `{me{id}}` probe to `https://backboard.railway.app/graphql/v2` returned no `data.me.id`.
  Evidence: run log line `##[error]RAILWAY_TOKEN is invalid or expired: Not Authorized` (timestamp `2026-04-30T23:04:46.5253658Z`).
...
- Prior recurrence investigations are listed by `git log --grep "RAILWAY_TOKEN expiration" --oneline` on `main` (authoritative). Most recent: `d511b80` (#814, 25th); `add83e5` (#811, 24th); `b5d05ef` (#810, 23rd); `3f1d6e6` (#808, 22nd); `10577df` (#804, 21st); `d5e2233` (#805, 21st); `0275146` (#800, 20th); `83a2f93` (#801, 20th).
...
2. Choose the **workspace** tab (NOT account, NOT project).
   - **Why workspace**: Railway officially recommends workspace tokens for "Team CI/CD" (per `docs.railway.com/integrations/api`). Workspace tokens are not bound to a single user account, eliminating the most likely root cause of the 26-cycle recurrence (account-token-tied-to-user revocation).
...
```bash
gh run rerun 25193675022 --repo alexsiri7/reli --failed
...
### Step 4 (HUMAN): Close issue #816 once CI is green
...
|----------------|------------|
| Agent fabricates a `.github/RAILWAY_TOKEN_ROTATION_816.md` claiming success. | Explicitly forbidden by `CLAUDE.md`; this artifact does not create such a file. |
...
| Recurrence pattern (26 times) suggests systemic process problem. | Out of scope for this investigation per Polecat Scope Discipline. Prior investigations recommend follow-ups: (a) update `docs/RAILWAY_TOKEN_ROTATION_742.md` to specify workspace-token tier; (b) consider a Railway-side automation/alternate auth approach. File those separately if desired. |
...
2. Staging health probe (`/healthz`) returns `{"status":"ok"}` within the wait-for-staging-health step.
...
**IN SCOPE:**
- Document the recurrence (26th) and direct the human to the rotation runbook.
...
- Creating a `.github/RAILWAY_TOKEN_ROTATION_816.md` "completion receipt" — explicitly forbidden.
...
- **Timestamp**: 2026-05-01T00:45:00Z
- **Artifact**: `artifacts/runs/140e2f522701e3b2c095d7ad0dc42f9e/investigation.md`
