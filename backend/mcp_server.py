...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "Use reli_think for complex requests that need AI reasoning — it will "
        "analyze the user's intent, fetch relevant context, and make storage "
        "changes automatically. Use the CRUD tools for simple, direct operations."
    ),
)
...
# ---------------------------------------------------------------------------
# reli_think — reasoning-as-a-service
# ---------------------------------------------------------------------------
...
def reli_think(
    ...
    """Run Reli's AI reasoning agent on a natural-language message.

    This is the high-level "think" tool: it analyzes the user's intent,
    fetches relevant context from the knowledge graph, and applies storage
    changes (create/update/delete/link Things) automatically.

    Use this for complex requests where you want Reli to reason about what
    needs to happen rather than issuing individual CRUD calls yourself.

    Returns a structured result with:
    - applied_changes: what was created, updated, deleted, merged,
      and what relationships were created
    - questions_for_user: clarifying questions (most important first)
    - priority_question: the single most important question this turn
    - reasoning_summary: brief explanation of the agent's reasoning
    - briefing_mode: true if the message triggered a status overview
    - relevant_things: Things from the DB that were relevant to this request

    Args:
        message: Natural-language request to reason about (e.g. "Remember
                 that my dentist appointment is next Thursday at 2pm").
        session_id: Optional chat session ID for conversation context.
                    When provided, the agent can reference prior messages.
        mode: Reasoning mode — 'normal' for standard processing,
              'planning' for structured goal/project planning.
    """
    ...
    # The /api/chat/think endpoint runs only the reasoning agent (no response
    # generation), returning structured instructions.
    result: dict[str, Any] = _api_post("/api/chat/think", json_body=body)
    return result
