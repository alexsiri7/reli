...
def _patch_agents(
    ...
    """Return a list of context managers patching all agent functions."""
...
class TestChatPipeline:
    ...
    async def test_chat_passes_context_fetcher_to_reasoning(self, async_client):
        """Reasoning agent receives a context_fetcher callable."""
        patches = _patch_agents()
        with patches[0] as mock_reasoning, patches[1]:
            await async_client.post(
                "/api/chat",
                json={"session_id": "ctx-sess", "message": "Hello"},
            )
            # Verify context_fetcher was passed
            call_kwargs = mock_reasoning.call_args
            assert call_kwargs.kwargs.get("context_fetcher") is not None
