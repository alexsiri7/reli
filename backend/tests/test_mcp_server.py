...
class TestMcpMetadata:
    ...
    def test_has_all_prompts(self) -> None:
...
# ---------------------------------------------------------------------------
# Prompt resources — Phase 2
# ---------------------------------------------------------------------------


class TestPromptResources:
    def test_thing_creation_guidance_returns_content(self) -> None:
        result = thing_creation_guidance()
        assert result == _THING_CREATION_GUIDANCE
        assert "type_hint" in result
        assert "Open Questions" in result
        assert "priority" in result.lower()
        assert "checkin_date" in result

    def test_relationship_patterns_returns_content(self) -> None:
        result = relationship_patterns()
        assert result == _RELATIONSHIP_PATTERNS
        assert "works_with" in result
        assert "depends_on" in result
        assert "belongs_to" in result
        assert "from_thing" in result

    def test_type_hint_usage_returns_content(self) -> None:
        result = type_hint_usage()
        assert result == _TYPE_HINT_USAGE
        assert "task" in result
        assert "project" in result
        assert "person" in result
        assert "preference" in result

    def test_proactive_surfacing_rules_returns_content(self) -> None:
        result = proactive_surfacing_rules()
        assert result == _PROACTIVE_SURFACING_RULES
        assert "birthday" in result
        assert "deadline" in result
        assert "recurring" in result.lower()
        assert "one-shot" in result.lower()

    def test_thing_creation_guidance_covers_all_type_hints(self) -> None:
        """Verify the prompt covers all type_hint values from the system."""
        expected_types = [
            "task",
            "note",
            "idea",
        ...
        for t in expected_types:
            assert t in result, f"type_hint '{t}' missing from thing_creation_guidance"

    def test_proactive_surfacing_covers_date_keys(self) -> None:
        """Verify the prompt documents all recognized date keys."""
        recurring_keys = ["birthday", "anniversary", "dob"]
        oneshot_keys = ["deadline", "due_date", "event_date", "start_date", "end_date"]
        result = proactive_surfacing_rules()
        for k in recurring_keys:
            assert k in result, f"Recurring key '{k}' missing from proactive_surfacing_rules"
        for k in oneshot_keys:
            assert k in result, f"One-shot key '{k}' missing from proactive_surfacing_rules"
...
class TestIntegration:
    ...
    def test_crud_lifecycle(self, api_server) -> None:  # type: ignore[no-untyped-def]
    ...
    def test_relationship_lifecycle(self, api_server) -> None:  # type: ignore[no-untyped-def]
