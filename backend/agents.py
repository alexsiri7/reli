...
def _client(api_key: str | None = None, base_url: str | None = None) -> AsyncOpenAI:
    return AsyncOpenAI(
        api_key=api_key or REQUESTY_API_KEY,
        base_url=base_url or REQUESTY_BASE_URL,
    )
...
async def run_context_agent(
    ...
    raw = None
    model_to_use = context_model or REQUESTY_MODEL
...
async def run_reasoning_agent(
    ...
    raw = await _chat(
        messages,
        model=reasoning_model or REQUESTY_REASONING_MODEL,
        response_format={"type": "json_object"},
        usage_stats=usage_stats,
        api_key=api_key,
        base_url=base_url,
