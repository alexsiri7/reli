...
`MCP_API_TOKEN` is the bearer token for `/mcp`, and `WEB_UI_PASSWORD` is the HTTP Basic password
for the web view at `/` and the `/api` routes behind it. Leaving either empty does not open what it
guards: every request gets a 401 until it is set. `/healthz` stays open either way, so a missing
...
`http://localhost:8000/mcp`, which requires `Authorization: Bearer $MCP_API_TOKEN`, and the web view
