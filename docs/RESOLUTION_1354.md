...
Issue #1354 was a false positive from the `pipeline-health-cron.sh` monitoring script. The cron detected `HTTP 000` (connection refused) at 2026-08-28 03:02:04 UTC across all 3 retry attempts. This is consistent with a Railway container restart exceeding the 3×30s (~90s) retry window — no actual application outage occurred.
...
This is the seventeenth instance of this false positive:
...
| #1354 | 2026-08-28 | Retry window too short (3×30s) | This resolution |

## Evidence

- **No code changes on Aug 27–28 2026** that would trigger a deployment. Last commit was `a4d9a13` (2026-08-26, docs only).
