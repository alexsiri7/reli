...
Issue #1338 was a false positive from the `pipeline-health-cron.sh` monitoring script. The cron detected `HTTP 000` (connection refused) at 2026-08-18 03:02:01 UTC across all retry attempts. This is consistent with a Railway container restart exceeding the 6×20s (~100–160s) retry window — no actual application outage occurred.
...
This is the fourteenth instance of this false positive:
...
| #1338 | 2026-08-18 | Retry window still too short (6×20s) | This resolution |

## Evidence

- **No code changes on Aug 17–18 2026** that would trigger a deployment or cause a startup failure. Last commit was `4f4f602` (MCP error message improvement).
...
- Issue #1338 — this issue (6×20s retry insufficient; recommend 12×20s)
