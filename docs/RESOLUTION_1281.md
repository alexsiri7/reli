...
Issue #1281 was a false positive from the `pipeline-health-cron.sh` monitoring script. The cron detected `HTTP 000` (connection refused) at 2026-07-02 03:01:56 UTC across all retry attempts. This is consistent with a Railway container restart exceeding the 6x20s (~100-160s) retry window — no actual application outage occurred.
...
| #1281 | 2026-07-02 | Retry window still too short (6x20s) | This resolution |
...
- Issue #1283 — same pattern (6x20s retry insufficient for July 4 restart)
- Issue #1284 — same pattern (6x20s retry insufficient for July 5 restart)
