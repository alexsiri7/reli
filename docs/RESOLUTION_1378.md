...
Issue #1378 was a false positive from the `pipeline-health-cron.sh` monitoring script. The cron detected `HTTP 000` (connection refused) at 2026-09-03 03:02:01 UTC across all 3 retry attempts. This is consistent with a Railway container restart exceeding the 3×30s (~90s) retry window — no actual application outage occurred. This is the twenty-first instance of this false positive pattern.
...
This is the twenty-first instance of this false positive (HTTP 000 / retry-window pattern only —
issues #1358 and #1361 track a separate lag-detection alert type; #1178/#1179 counted as one event):
...
| #1378 | 2026-09-03 | Retry window too short (3×30s) | This resolution |

## Evidence

- **Site confirmed live** — `curl https://reli.interstellarai.net/healthz` returns HTTP 200 `{"status":"ok","service":"reli"}` at time of investigation.
- **No code changes on 2026-09-02 to 2026-09-03** that would trigger a crash. Last deployment was commit `5bf68cc` (docs: add resolution for false positive deploy alert #1376), which is non-breaking documentation only.
...
- **Detection at 03:02:01 UTC** — matches the pattern of Railway periodic container restarts occurring independently of deployments.
...
- Issue #1376 — same pattern (recommend 12×20s, twentieth instance)
