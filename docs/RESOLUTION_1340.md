...
Issue #1340 was a false positive from the `pipeline-health-cron.sh` monitoring script. The cron detected `HTTP 000` (connection refused) at 2026-08-20 03:01:57 UTC across all retry attempts. This is consistent with a Railway container restart exceeding the 6×20s (~100–160s) retry window — no actual application outage occurred.
...
This is the fifteenth instance of this false positive:
...
| #1285 | 2026-07-06 | Retry window still too short (6×20s) | Documented in RESOLUTION_1285 |
| #1338 | 2026-08-18 | Retry window still too short (6×20s) | Documented in RESOLUTION_1338 |
| #1340 | 2026-08-20 | Retry window still too short (6×20s) | This resolution |

## Evidence

- **No code changes on Aug 19–20 2026** that would trigger a deployment or cause a startup failure. Last commit was `6bbb3f6` (false positive documentation for #1338).
...
Increase the retry window in `pipeline-health-cron.sh` (interstellarai.net repo) from 6×20s (~160s coverage) to 12×20s (~240–340s coverage). This covers Railway worst-case restarts with significant margin.
...
# RECOMMENDED (~240–340s coverage — 12 attempts × 20s delay + 10s max-time per call)
...
- Issues #1280–#1285 — same pattern (6×20s retry insufficient)
- Issue #1338 — same pattern (6×20s retry insufficient; recommend 12×20s)
