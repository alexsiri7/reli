...
Issue #1284 was a false positive from the `pipeline-health-cron.sh` monitoring script. The cron detected `HTTP 000` (connection refused) at 2026-07-05 03:01:55 UTC across all retry attempts. This is consistent with a Railway container restart exceeding the 6×20s (~100–160s) retry window — no actual application outage occurred.
...
This is the twelfth instance of this false positive:
...
| #1284 | 2026-07-05 | Retry window still too short (6×20s) | This resolution |

## Evidence

- **No code changes on July 4–5 2026** that would trigger a deployment or cause a startup failure.
